...
import { ShoppingCart, Plus, Search, Filter, CheckCircle2, FileText, X } from 'lucide-react';
...
export function SalesPage() {
  ...
  const getStatusBadgeVariant = (status: SalesOrder['status']) => {
    switch (status) {
      case 'INVOICED':
        return 'success';
      case 'CONFIRMED':
  ...
  const handleCreateOrder = (e: React.FormEvent) => {
    e.preventDefault();
    const customer = INITIAL_CONTACTS.find((c) => c.id === customerId);
    ...
    const newOrder: SalesOrder = {
      id: `so-${Date.now()}`,
      orderNumber: `SO-2026-00${salesOrders.length + 1}`,
    ...
    setOrderNotice(`Order ${newOrder.orderNumber} created and confirmed for ${customer.name}.`);
    setTimeout(() => setOrderNotice(null), 5000);
  };

  const handleConfirmOrder = (orderId: string) => {
    setSalesOrders(
      salesOrders.map((o) => (o.id === orderId ? { ...o, status: 'CONFIRMED' } : o))
    );
  };

  const handleGenerateInvoice = (orderId: string) => {
    setSalesOrders(
      salesOrders.map((o) => (o.id === orderId ? { ...o, status: 'INVOICED' } : o))
    );
    setOrderNotice(`Customer Invoice generated for order ${orderId}. Linked to Accounts Receivable.`);
    setTimeout(() => setOrderNotice(null), 5000);
  };
    ...
    <div className="mx-auto max-w-dashboard space-y-5 p-4 lg:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-surface-border pb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold tracking-tight text-navy-900">Sales Orders</h1>
          </div>
          <p className="mt-1 text-xs text-navy-400">
            Manage customer quotations, sales orders, and automated customer invoice generation.
          </p>
        </div>
        <Button
          variant="primary"
      ...
      {orderNotice && (
        <div className="flex items-center justify-between rounded-md border border-brand-200 bg-brand-50/70 p-3 text-xs text-brand-900">
          ...
          <button onClick={() => setOrderNotice(null)} className="text-brand-600 hover:text-brand-900">
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {['ALL', 'CONFIRMED', 'INVOICED', 'DRAFT'].map((st) => (
            <button
              key={st}
          ...
          <TableBody>
            {filteredOrders.length === 0 ? (
              ...
              filteredOrders.map((order) => (
                <TableRow key={order.id} className="cursor-pointer hover:bg-surface-secondary/60">
                  ...
                  <TableCell className="text-xs text-navy-500">{order.orderDate}</TableCell>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-900/50 p-4">
          <div className="w-full max-w-lg rounded-lg border border-surface-border bg-white p-6 shadow-xl">
            <div className="flex items-center justify-between border-b border-surface-border pb-3">
              <h2 className="text-base font-bold text-navy-900">Create Sales Order</h2>
              <button
            ...
            <form onSubmit={handleCreateOrder} className="mt-4 space-y-4">
              <div>
                ...
                >
                  {INITIAL_CONTACTS.filter((c) => c.type === 'CUSTOMER' || c.type === 'BOTH').map((c) => (
              ...
              <div className="rounded-md bg-surface-secondary p-3 text-xs space-y-1">
                <div className="flex justify-between text-navy-500">
                  ...
                  <span className="font-mono">
                    ₹{((INITIAL_PRODUCTS.find((p) => p.id === productId)?.salesPrice || 0) * quantity).toLocaleString('en-IN')}
                  </span>
                </div>
                <div className="flex justify-between text-navy-500">
                  <span>GST (18%):</span>
                  <span className="font-mono">
                    ₹{Math.round(((INITIAL_PRODUCTS.find((p) => p.id === productId)?.salesPrice || 0) * quantity) * 0.18).toLocaleString('en-IN')}
                  </span>
                </div>
                <div className="flex justify-between font-bold text-navy-900 border-t border-surface-border pt-1">
                  <span>Grand Total:</span>
                  <span className="font-mono text-brand-700">
                    ₹{Math.round(((INITIAL_PRODUCTS.find((p) => p.id === productId)?.salesPrice || 0) * quantity) * 1.18).toLocaleString('en-IN')}
                  </span>
                </div>
              </div>
          ...
          <div className="w-full max-w-md rounded-lg border border-surface-border bg-white p-5 shadow-xl space-y-3">
            <div className="flex items-center justify-between border-b border-surface-border pb-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-navy-900">{selectedOrder.orderNumber}</h3>
              </div>
            ...
            <div className="text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-navy-900">{selectedOrder.customerName}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="border-t border-surface-border pt-2">
                <span className="font-semibold text-navy-800">Line Items:</span>
